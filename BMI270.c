...
int main(void)
{
  ...
  while (1)
  {
	  // 1. Set Accel Config: ODR = 100Hz, Filter = Normal
	  BMI270_WriteReg(0x40, 0xA8);
	  HAL_Delay(10);

	  // 2. Set Gyro Config: ODR = 100Hz, Filter = Normal
	  BMI270_WriteReg(0x42, 0xA9);
	  HAL_Delay(10);

	  // 3. Force Power Control: Enable Accel (bit 2) and Gyro (bit 1) and Temperature (bit 3)
      ...
      if (user_spi_read(0x0C, sensor_data, 12, NULL) == 0) {
          ...
          // Print Accel values (Divide by 16384.0 for 'g' units if range is +/-2g)
          printf("ACC: X=%d Y=%d Z=%d\r\n", acc_x, acc_y, acc_z);
      }

      HAL_Delay(100); // 10Hz update for clear reading
    /* USER CODE END WHILE */
  }
    /* USER CODE BEGIN 3 */
  /* USER CODE END 3 */
